...
class TextRNN(object):
    ...
    def rnn(self):
        ...
        with tf.name_scope("rnn1"):
            # 多层rnn网络
            scale = 1.0 / math.sqrt(TRNNConfig.embedding_dim + TRNNConfig.hidden_dim) / 3.0
            lstm_init = tf.random_uniform_initializer(-scale, scale)
            with tf.variable_scope("rnn1v", initializer = lstm_init):
                ...
                # inputs：一个三维的变量,[batch_size,time_step,input_size],搭配time_major=False。
        ...
        with tf.name_scope("sum"):
            sum1 = tf.reduce_mean(_outputs_1, axis=1)
            sum2 = tf.reduce_mean(_outputs_2, axis=1)

        with tf.name_scope('concat'):
            result = tf.concat([sum1, sum2], 1)

        with tf.name_scope("score"):
            # 全连接层，后面接dropout以及relu激活
            fc = tf.layers.dense(result, self.config.hidden_dim,
