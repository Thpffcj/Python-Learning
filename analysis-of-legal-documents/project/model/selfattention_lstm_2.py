    ...
    def rnn(self):
        ...
        with tf.name_scope("bilstm"):
            with tf.variable_scope("bi-lstm1"):
                ...
                _last1 = tf.add(_outputs_1[0], _outputs_1[1]) / 2
            ...
            with tf.variable_scope("bi-lstm2"):
                ...
                _outputs_2, state_2 = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell_fw, cell_bw=cell_bw,
                                                                      ...
                                                                      initial_state_fw=init_state_fw,
                                                                      initial_state_bw=init_state_bw)
                _last2 = tf.add(_outputs_2[0], _outputs_2[1]) / 2

        with tf.name_scope("attention"):
            with tf.variable_scope("linear"):
                # 首先做线性变换，其中对input_2做两次线性变换
                newinput1 = tf.expand_dims(_last1, axis=3)
                newinput2 = tf.expand_dims(_last2, axis=3)
            ...
            with tf.variable_scope("selfattention"):
                t1 = tf.matmul(Q, K, adjoint_b=True) / math.sqrt(
        ...
        with tf.name_scope("concat"):
            concat = tf.concat([gmp1, gmp3, gmp4], 1)
